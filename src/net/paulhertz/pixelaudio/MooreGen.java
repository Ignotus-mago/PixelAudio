 ...
 * Width and height must be equal powers of 2. You can also call MooreGen(int depth) and width and height 
 * will equal Math.pow(2, depth). The Moore curve is generated by rotating and reflecting 4 copies of 
 * a Hilbert curve with a depth of recursion one level lower.
 * 
 * @see PixelMapGen
 * @see HilbertGen
 * 
...
public class MooreGen extends PixelMapGen {
	...
	public boolean requireValidDimensions(int width, int height) {
		...
		if (width != height) {
			throw new IllegalArgumentException("MooreGen Error: Width and height must be equal.");
		}
