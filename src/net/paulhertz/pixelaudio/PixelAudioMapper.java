 ...
 * PixelAudioMapper, which just works with the arrays of audio sample or image pixel data that they provide. 
 ...
 * transcoding values to other formats.  
 * </p>
 * <p>
 * <h3>Image</h3>
 * <pre>
 ...
 *  </pre>
 * </p><p>
 * The LUTs are generated by a subclass of <code>PixelMapGen</code> that plugs into the <code>PixelAudioMapper</code> constructor. 
 * Each <code>PixelMapGen</code> subclass generates: 1. a set of coordinates for the path traced by the signal over the image, 
 * 2. <code>imageToSignalLUT</code>, for mapping from image to signal, and 3. <code>signalToImageLUT</code> for mapping from signal to image. 
...
public class PixelAudioMapper {
	...
	*/
	...
	public float[] pluckPixelsAsAudio(int[] img, int x, int y, int length, ChannelNames fromChannel) {
		...
		case L: {
			for (int i = pos; i < pos + length; i++) {
				...
				samples[j++] = PixelAudio.map(brightness(rgb, hsbPixel), 0, 1, -1.0f, 1.0f);			// replace rgb with its value expression img[this.imageToSignalLUT[i]]
		...
		case H: {
			for (int i = pos; i < pos + length; i++) {
				int rgb = img[this.imageToSignalLUT[i]];
				samples[j++] = PixelAudio.map(hue(rgb, hsbPixel), 0, 1, -1.0f, 1.0f);
			}
			break;
		}
		case S: {
			for (int i = pos; i < pos + length; i++) {
				int rgb = img[this.imageToSignalLUT[i]];
				samples[j++] = PixelAudio.map(saturation(rgb, hsbPixel), 0, 1, -1.0f, 1.0f);
			}
			break;
		}
		case R: {
			for (int i = pos; i < pos + length; i++) {
				int rgb = img[this.imageToSignalLUT[i]];
				samples[j++] = PixelAudio.map(((rgb >> 16) & 0xFF), 0, 255, -1.0f, 1.0f);
			}
			break;
		}
		case G: {
			for (int i = pos; i < pos + length; i++) {
				int rgb = img[this.imageToSignalLUT[i]];
				samples[j++] = PixelAudio.map(((rgb >> 8) & 0xFF), 0, 255, -1.0f, 1.0f);
			}
			break;
		}
		case B: {
			for (int i = pos; i < pos + length; i++) {
				int rgb = img[this.imageToSignalLUT[i]];
				samples[j++] = PixelAudio.map((rgb & 0xFF), 0, 255, -1.0f, 1.0f);
			}
			break;
		}
		case A: {
			for (int i = pos; i < pos + length; i++) {
				int rgb = img[this.imageToSignalLUT[i]];
				samples[j++] = PixelAudio.map(((rgb >> 24) & 0xFF), 0, 255, -1.0f, 1.0f);
			}
			break;
		}
		case ALL: {
	...
	public void plantPixels(int[] sprout, int[] img, int x, int y, int length, ChannelNames toChannel) {
		...
		case L: {
			for (int i = pos; i < pos + length; i++) {
				...
				rgb = Color.HSBtoRGB(hsbPixel[0], hsbPixel[1], brightness(sprout[j], hsbPixel));
		...
		case H: {
			for (int i = pos; i < pos + length; i++) {
				int rgb = img[this.imageToSignalLUT[i]];
				Color.RGBtoHSB((rgb >> 16) & 0xff, (rgb >> 8) & 0xff, rgb & 0xff, hsbPixel);
				rgb = Color.HSBtoRGB(hue(sprout[j], hsbPixel), hsbPixel[1], hsbPixel[2]);
				img[this.imageToSignalLUT[i]] = rgb;
				j++;
			}
			break;
		...
		case S: {
			for (int i = pos; i < pos + length; i++) {
				int rgb = img[this.imageToSignalLUT[i]];
				Color.RGBtoHSB((rgb >> 16) & 0xff, (rgb >> 8) & 0xff, rgb & 0xff, hsbPixel);
				rgb = Color.HSBtoRGB(hsbPixel[0], saturation(sprout[j], hsbPixel), hsbPixel[2]);
				img[this.imageToSignalLUT[i]] = rgb;
				j++;
			}
			break;
	...
	public void plantSamples(int[] sprout, float[] sig, int pos, int length, ChannelNames fromChannel) {
		...
		case L: {
			for (int i = pos; i < pos + length; i++) {
				sig[i] = PixelAudio.map(brightness(sprout[j++], hsbPixel), 0, 1, -1.0f, 1.0f);
			}
			break;
		}
		case H: {
			for (int i = pos; i < pos + length; i++) {
				sig[i] = PixelAudio.map(hue(sprout[j++], hsbPixel), 0, 1, -1.0f, 1.0f);
			}
			break;
		}
		case S: {
			for (int i = pos; i < pos + length; i++) {
				sig[i] = PixelAudio.map(saturation(sprout[j++], hsbPixel), 0, 1, -1.0f, 1.0f);
			}
			break;
		}
		case R: {
