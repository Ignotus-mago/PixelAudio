 ...
 * ==================
 *
 * 1. Launch TutorialOne_03_Drawing. The display window opens with a pre-loaded file, "Saucer_mixdown.wav",
 ...
 * by the PixelMapGen multigen and managed by the PixelAudioMapper mapper.
 * This particular PixelMapGen plots time from top to bottom, left to right, in eight rows.
 *
 * 2. Click on the image or press the spacebar with the cursor over the image to play
 * a Sampler instrument sound.
 *
 * 3. Press '/' to enable drawing. Drag the mouse to draw a line. Vary the speed of your gesture:
 * it will be recorded to a PACurveMaker instance. When you release the
 * mouse, a brushstroke appears.
 *
 * 4. Activate the brushstroke with hover-spacebar or a mouse click. The Sampler instrument plays new brushstokes,
 ...
 *
 * 5. The Sampler instrument usually sounds good with ALL_POINTS or REDUCED_POINTS. It may produce distortion with CURVE_POINTS.
 *   -- Change the 'epsilon' value that controls the number of reduced points with the '<' or '>' keys.
 *   -- Change the number of curve steps with the '[' or ']' keys.
 *
 ...
 * more energy than most instrumental samples and will distort more easily.
 *
 * 8. To adjust Sampler audio output:
 ...
 * ===================
 *
 * 1. To make a brushstroke use the Granular instrument, hover over it and press 't'.
 ...
 * Press 't' again to switch the curve back to use the Sampler instrument.
 *
 * 2. When the Granular instrument plays a curve, it steps through the audio buffer in a
 ...
 * may be close to linear audio buffer playback. It all depends on the PixelAudioMap you use.
 *
 * 3. Various keys can change the sound of the Granular instrument. The granular instrument
 ...
 *      gesture timing and overlaps each audio sample by the time offset hopSamples.
 *
 * 4. Some adjustments will make the granular samples sound more like the underlying buffer.
 ...
 * ================
 *
 * The PAGranularInstrumentDirector class manages the high level processes
