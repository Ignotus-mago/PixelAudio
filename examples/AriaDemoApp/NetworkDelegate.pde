 ...
 * Used by applications that implement the PANetworkClientINF. However, we can't declare that our 
 * main class in Processing implements a particular interface. It implicitly extends PApplet, but we 
 * access its declaration to tack on "implements PANetworkClientINF". My workaround here is to replace
 * all occurrences "PANetworkClientINF" with the name of the app. If you want to use this class as is
 * in your own sketch, just replace all occurrences of "AriaDemoApp" with the name of your app. 
 *
 * This is not the only way to solve the problem. We could also just delete all references to the host
 * app, changing the constructors for the NetworkDelegate class. Since it's an interior class of the 
 * main app, it already can access methods in the host app, without needing an interface declaration. 
 * 
 * @see PANetworkClientINF
 */
public class NetworkDelegate {
  private PApplet parent;
  private AriaDemoApp app;
  private OscP5 osc;
  ...
  public NetworkDelegate(AriaDemoApp app, String remoteFromAddr, String remoteToAddr, int inPort, int outPort) {
    this.app = app;
  ...
  public NetworkDelegate(AriaDemoApp app, String remoteFromAddr, String remoteToAddr) {
    this.app = app;
    this.parent = app.getPApplet();
    this.osc = new OscP5(parent, inPort);
    this.remoteFromAddress = remoteFromAddr;
  ...
  public NetworkDelegate(AriaDemoApp app) {
    this.app = app;
    this.parent = app.getPApplet();
    this.osc = new OscP5(parent, inPort);
    init();
